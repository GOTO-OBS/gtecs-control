...
from testScheduler import Scheduler
from tecs_modules.asyncio_protocols import SimpleProtocol
...
class Pilot:
    ...
    async def nightMarshall(self, restart=False):
        ...
        Not implemented yet. Thinking about best way.
        """
        pass
    ...
    async def nightPlan(self):
        ...
        cmd = [os.path.join(script_dir, 'start_of_night_1.py')]
        await self.startScript('SN1', SimpleProtocol, cmd)

        cmd = [os.path.join(script_dir, 'start_of_night_2.py')]
        # await self.startScript('SN2', SimpleProtocol, cmd)
    ...
    async def cancelRunningScript(self):
        ...
        if self.runningScriptTransport is not None:
            ...
            if self.runningScriptTransport.get_returncode() is None:
                self.log.info("Killing {}".format(self.currJob[0]))
    ...
    async def observe(self):
        ...
        while True:
            ...
            if (self.currJob != self.nextJob) and self.nextJob[0] is not None:
                ...
                # does nothing if running job completed
                ...
                cmd = [os.path.join(script_dir, 'observe.py'), name, str(duration)]
...
if __name__ == "__main__":
    ...
    stopSignal = pilot.waitForNightsEnd(Time.now() + 70*u.s)
